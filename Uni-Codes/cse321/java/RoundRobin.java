...
class RoundRobin {
    public static void main(String[] args) {
        ...
        while (sum == 0) {
            for (int i = 0; i < n; i++) {
                if (brusttime[i] > quantumValue) {
                    ...
                    for (int j = 0; j < n; j++)
                        if ((j != i) && (brusttime[j] != 0))
                            waitingtime[j] += quantumValue;
                } else {
                    for (int j = 0; j < n; j++)
                        if ((j != i) && (brusttime[j] != 0))
                            waitingtime[j] += brusttime[i];
                    brusttime[i] = 0;
                }
            }
            for (int k = 0; k < n; k++)
        ...
        for (int i = 0; i < n; i++)
            turnaroundTime[i] = waitingtime[i] + proc[i];
        System.out.println("process\t\t BT\tWT\tTAT\tCT");
        for (int i = 0; i < n; i++)
            System.out.println("Process: " + (i + 1) + "\t" + proc[i] + "\t" + waitingtime[i] + "\t" + turnaroundTime[i]
                    + "\t" + turnaroundTime[i]);
        ...
        for (int j = 0; j < n; j++)
            avg_turnaroundTime += turnaroundTime[j];
